...
effect on the treated. However, we have updated the package to also
meaningfully handle the case where interest lies in using the
population weights (e.g., weighting of comparison and treatment cases
to estimate the population average treatment effect.) The main
...
The \texttt{twang} package aims to compute from the data estimates of
the propensity scores which yield accurate causal effect estimates, 
check the quality of the weights by assessing whether or not they
...
of the treatment and weighted control group. 
The \texttt{ps}
...
standardized mean difference of the \textit{E}ffect \textit{S}ize) and
...
%rules consist of a
%statistic that is evaluated on each of the pre-treatment variables
%that was used in the call to \texttt{ps()}, and a method for
%summarizing the statistics across the pretreatment
%variables. \texttt{es} stands for
%``standardized \underline{e}ffect 
%\underline{s}ize'' and \texttt{ks} stands for the
%Kolmogorov-Smirnov statistic.  The included \texttt{stop.methods}
%either use the maximum (\texttt{.max}) or the mean (\texttt{.mean}) of
%these statistics 
%as the overall measure of balance to be optimized. 
...
\Sexpr{ps.lalonde$desc$es.mean.ATT$n.trees} 
iterations minimized
 the average effect size difference and
\Sexpr{ps.lalonde$desc$ks.max.ATT$n.trees} iterations minimized
...
\Sexpr{round(ps.lalonde$desc$es.mean.ATT$ess.ctrl,1)} or
\Sexpr{round(ps.lalonde$desc$ks.max.ATT$ess.ctrl,1)} of the comparison
...
The \texttt{plot()} method can generate useful
...
hypothesis would be true). Setting \texttt{plots = 4} or \texttt{plots="t"} 
...
\Sexpr{100*lalonde.balance$ks.max.ATT[5,1]}\% of the treatment
group had no degree while
\Sexpr{100*lalonde.balance$ks.max.ATT[5,3]}\% of the comparison
...
\texttt{es.mean} and \texttt{ks.max}.  
...
\texttt{ejectfrac} and \texttt{ves1proc} were all significantly
...
%is to represent in the analysis. 

%This second component is a
...
Note that the weights $p$ are equivalent to the propensity score if we
...
@
\end{center}
\caption{Optimization of \texttt{es.mean.ATE} and
\texttt{ks.max.ATE} for nonresponse weighting of egsingle data. The
...
statistic is \Sexpr{egsingle.ps$desc$ks.max.ATE$n.trees} and the
optimal number of iterations for gbm to minimize the average effect
size is \Sexpr{egsingle.ps$desc$es.mean.ATE$n.trees}.  The weights
...
The final steps are to find the ATE weights

<<>>=
egsingle.one$wgt <- get.weights(egsingle.ps, stop.method="ks.max")
@

and select only the records with an observed outcome.
...
\begin{equation}
 f(\mathbf{x} | t=0) = w(\mathbf{x}) f(\mathbf{x}), \mbox{ and}
  \label{eq:balance3}
\end{equation}
Again using Bayes Theorem we obtain $w(\mathbf{x}) =
1/f(\mathbf{x}|t=1)$ for the treatment group and $w(\mathbf{x}) = 1/
f(\mathbf{x} | t = 0)$ for the control group.  

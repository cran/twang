...
\author{Lane Burgette, Beth Ann Griffin and Dan McCaffrey 
  \footnote{The development of this software and tutorial was funded
    by National Institute of Drug Abuse grant number 1R01DA015697 (PI: McCaffrey).} 
\\RAND Corporation}
...
to handle more than two treatment conditions through the new
...
circles in this plot would be solid. 
...
all other treatment groups.  Note that KS
$p$-values are not 
available for ATE in the multiple treatment setting, and the 
\texttt{plots} argument therefore may not be set to 5.  
...
which provides a nice, simple summary of balance across the groups.
...
multiple treatments, this function returns a lot of information. For
...
Note in this case that the ``control'' columns (labeled \texttt{ct})
refer to every treatment group except the one that is considered the
treatment for a particular output.  For example, in the first table
that follows, \texttt{tx} refers to \texttt{community} treatment, and
\texttt{ct} refers to all treatments except \texttt{community}. 
...
the number of iterations (trees) selected for each model under each
stopping rule.
...
sizes (\texttt{ess.treat}) are similar as well.  Hence, we expect the
...
multiple treated setting is that we must be clear as to which treatment
...
The \texttt{bal.table} output is similar to the ATE case.  However, for ATT, we do not
need tables that give balance for the \texttt{treatATT} category
against itself.  
